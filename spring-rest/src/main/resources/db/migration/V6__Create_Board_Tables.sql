...
-- 2. CHMM_BOARD_CATEGORY (게시판 카테고리 관리)
-- 특정 게시판(BRD_ID)에 종속된 카테고리 분류
CREATE TABLE CHMM_BOARD_CATEGORY (
    CATEGORY_ID     SERIAL PRIMARY KEY,          -- 카테고리 일련번호
    BRD_ID          VARCHAR(20) NOT NULL,       -- 게시판 마스터 ID (FK)
    CATEGORY_NM     VARCHAR(100) NOT NULL,      -- 카테고리 명
    SORT_ORD        INTEGER DEFAULT 0,          -- 출력 순서
    USE_YN          CHAR(1) DEFAULT '1' NOT NULL, -- 사용 여부
...
CREATE TABLE CHMM_BOARD (
    ...
    BRD_ID          VARCHAR(20) NOT NULL,       -- 게시판 마스터 ID (FK)
    CATEGORY_ID     INTEGER,                    -- 카테고리 일련번호 (FK, 분류가 없을 경우 NULL 가능)
    ...
    SYS_UPDATE_USER_ID VARCHAR(50),
    FOREIGN KEY (BRD_ID) REFERENCES CHMM_BOARD_MASTER(BRD_ID) ON DELETE CASCADE,
    FOREIGN KEY (CATEGORY_ID) REFERENCES CHMM_BOARD_CATEGORY(CATEGORY_ID) ON DELETE SET NULL
);

-- 조회 성능 최적화 인덱스
CREATE INDEX IX_CHMM_BOARD_BRD_CAT ON CHMM_BOARD (BRD_ID, CATEGORY_ID);
